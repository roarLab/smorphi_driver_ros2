// --------------------------------------------------
// created by: Vinu Sivanantham  16/09/2023
// ---------------------------------------------------
...
{
public:
  ImuNode() : Node("imu_publisher_node")
  {
    //this->declare_parameter("serial_port", "/dev/smorphi_imu");
    //serial_port_ = this->get_parameter("serial_port").as_string();
    std::vector<rclcpp::Parameter> all_new_parameters{ \
      rclcpp::Parameter("if_debug_", true), \
      rclcpp::Parameter("serial_port_", "/dev/ttyUSB0"), \
      rclcpp::Parameter("serial_baud_", 115200), \
      rclcpp::Parameter("serial_timeout_", 20), \
      rclcpp::Parameter("device_type_", 1), \
      rclcpp::Parameter("frist_sn_", false), \
      rclcpp::Parameter("imu_topic_", "imu_data"), \
      rclcpp::Parameter("imu_frame_", "imu_link"), \
      rclcpp::Parameter("mag_pose_2d_topic_", "magnetic_pose_2d"), \
      rclcpp::Parameter("imu_trueEast_topic_", "imu_trueEast"), \
      rclcpp::Parameter("mag_topic_", "magnetic_field"), \
    ...
    mag_pose_pub_ = this->create_publisher<geometry_msgs::msg::Pose2D>("magnetic_pose_2d", 10);
    mag_pub_ = this->create_publisher<sensor_msgs::msg::MagneticField>("magnetic_field", 10);


try
    {
        serial_.setPort("/dev/smorphi_imu");
    ...
    {
      //RCLCPP_ERROR_STREAM_THROTTLE(this->get_logger(), *this->get_clock(), 1000, "Unable to open serial port " << "/dev/smorphi_imu");
      exit(0);
    }
    if (serial_.isOpen()) {
            RCLCPP_INFO(this->get_logger(), "Serial Port initialized");
        } else {
            //RCLCPP_ERROR_STREAM_THROTTLE(this->get_logger(), "Unable to initialize Serial port");
            exit(0);
        }
    timer_ = this->create_wall_timer(50ms, std::bind(&ImuNode::processLoop, this));
       // processLoop();
    }

    // Set up a timer to publish IMU data at 10Hz
...
private:
  void processLoop() {
        RCLCPP_INFO(this->get_logger(), "ahrs_driver::processLoop: start");

              while (rclcpp::ok()) {
            if (!serial_.isOpen()) {
                //RCLCPP_WARN_STREAM_SKIPFIRST("serial unopen");
            }
            //check head start
            ...
            if (if_debug_) {
                if (head_s != 1) {
                    //RCLCPP_ERROR_STREAM_THROTTLE("Read serial port time out! can't read pack head.");
                }
                std::cout << std::endl;
            ...
            //RCLCPP_INFO("My size is: %s", std::to_string(type_s).c_str());
            //RCLCPP_INFO("My info is: %s", std::to_string(head_type[0]).c_str());
            if (if_debug_) {
            ...
            if (head_type[0] != TYPE_IMU && head_type[0] != TYPE_AHRS && head_type[0] != TYPE_INSGPS &&
                ...
                //RCLCPP_WARN_STREAM_SKIPFIRST("head_type error: %02X", head_type[0]);
                continue;
            }
            //check head length
            ...
            if (head_type[0] == TYPE_IMU && check_len[0] != IMU_LEN) {
                //RCLCPP_WARN_STREAM_SKIPFIRST("head_len error (imu)");
                continue;
            } else if (head_type[0] == TYPE_AHRS && check_len[0] != AHRS_LEN) {
                //RCLCPP_WARN_STREAM_SKIPFIRST("head_len error (ahrs)");
                continue;
            } else if (head_type[0] == TYPE_INSGPS && check_len[0] != INSGPS_LEN) {
                //RCLCPP_WARN_STREAM_SKIPFIRST("head_len error (insgps)");
                continue;
            } else if (head_type[0] == TYPE_GROUND || head_type[0] == 0x50) // Unknown data to prevent recording failure
            {
                ...
                if (++read_sn_ != ground_sn[0]) {
                    if (ground_sn[0] < read_sn_) {
                        if (if_debug_) {
                            //RCLCPP_WARN_STREAM_SKIPFIRST("detected sn lost.");
                        }
            ...
            if (head_type[0] == TYPE_IMU) {
                ...
                if (CRC8 != imu_frame_.frame.header.header_crc8) {
                    //RCLCPP_WARN_STREAM_SKIPFIRST("header_crc8 error");
                    continue;
                }
                if (!frist_sn_) {
            ...
            if (head_type[0] == TYPE_IMU) {
                ...
                if (head_crc16 != CRC16) {
                    //RCLCPP_WARN_STREAM_SKIPFIRST("check crc16 faild(imu).");
                    continue;
                } else if (imu_frame_.frame.frame_end != FRAME_END) {
                    //RCLCPP_WARN_STREAM_SKIPFIRST("check frame end.");
                    continue;
                }
                ...
                if (head_crc16 != CRC16) {
                    //RCLCPP_WARN_STREAM_SKIPFIRST("check crc16 faild(ahrs).");
                    continue;
                ...
                if (head_crc16 != CRC16) {
                    //RCLCPP_WARN_STREAM_SKIPFIRST("check crc16 faild(insgps).");
                    continue;
            ...
            if (head_type[0] == TYPE_AHRS) {
                ...
                //imu_data.child.frame_id = "base_link";
...
serial::Serial serial_port_;
  ...
  FDILink::imu_frame_read imu_frame_;
        ...
        //topic
        std::string imu_topic_="imu_data", mag_pose_2d_topic_ = "mag";
  ...
  std::string imu_trueEast_topic_="trueeast";
  std::string mag_topic_="mag_t";
  // ros2 frame
  //std::string imu_frame_id_;
  // others
  double yaw_offset_;
...
void checkSN(int type)
  {
    ...
    {
    case TYPE_IMU:
      //RCLCPP_WARN_STREAM_SKIPFIRST_STREAM_SKIPFIRST(this->get_logger(),"TYPE_IMU");
      if (++read_sn_ != imu_frame_.frame.header.serial_num)
      {
        //RCLCPP_WARN_STREAM_SKIPFIRST_STREAM_SKIPFIRST(this->get_logger(),"inside_if");
        if ( imu_frame_.frame.header.serial_num < read_sn_)
        {
          sn_lost_ += 256 - (int)(read_sn_ - imu_frame_.frame.header.serial_num);
          if(if_debug_){
            //RCLCPP_WARN_STREAM_SKIPFIRST_STREAM_SKIPFIRST(this->get_logger(),"detected sn lost.");
          }
        }
        else
        {
...
int main(int argc, char* argv[])
{
  ...
  rclcpp::spin(std::make_shared<FDILink::ImuNode>());
  rclcpp::shutdown();
  return 0;
}
